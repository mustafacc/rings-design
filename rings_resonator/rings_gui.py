...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QProgressBar, QFileDialog, QMessageBox, QGroupBox, QSplitter,
    QFrame, QScrollArea, QToolBar, QCheckBox
)
...
class InteractiveGeometryPlot(QWidget):
    ...
    def setup_ui(self):
        ...
        self.show_labels.stateChanged.connect(lambda state: self.log_callback and self.log_callback(f"Labels display {'enabled' if state else 'disabled'}"))
        ...
        self.show_dimensions.stateChanged.connect(lambda state: self.log_callback and self.log_callback(f"Dimensions display {'enabled' if state else 'disabled'}"))
        ...
        self.show_coupling.stateChanged.connect(lambda state: self.log_callback and self.log_callback(f"Coupling display {'enabled' if state else 'disabled'}"))
    ...
    def show_placeholder(self):
        ...
        self.canvas.axes.text(0.5, 0.5, 'Configure Ring Parameters\nand See Live Geometry Preview',
    ...
    def update_geometry(self, ring_radii: List[float], coupling_coeffs: List[float], phase_shifts: List[float] = None):
        ...
        self.canvas.axes.plot([-x_extent, x_extent], [bus_top_y, bus_top_y], 
                             '#4CAF50', linewidth=6, alpha=0.8)
        self.canvas.axes.plot([-x_extent, x_extent], [bus_bottom_y, bus_bottom_y], 
                             '#F44336', linewidth=6, alpha=0.8)
        
        # Draw rings with enhanced styling
        ring_colors = ['#2196F3', '#4CAF50', '#FF9800', '#9C27B0', '#F44336', '#00BCD4', '#795548', '#607D8B']
        ...
        for i, (radius, (x, y)) in enumerate(zip(ring_radii, ring_positions)):
            ...
            if self.show_dimensions.isChecked():
                ...
                self.canvas.axes.text(x + radius/2, y + 8, f'R={radius:.1f}μm', 
            ...
            if self.phase_shifts[i] != 0:
                ...
                self.canvas.axes.text(x + radius + 15, y, phase_text, 
        ...
        if self.show_labels.isChecked():
            ...
            self.canvas.axes.text(-label_x_offset, bus_top_y, 'INPUT', ha='center', va='center',
            ...
            self.canvas.axes.text(label_x_offset, bus_top_y, 'THROUGH', ha='center', va='center',
                                fontsize=12, color='#4CAF50', weight='bold',
                                bbox=dict(boxstyle="round,pad=0.3", facecolor='#2e2e2e', edgecolor='#4CAF50'))
            ...
            self.canvas.axes.text(drop_x, bus_bottom_y, 'DROP', ha='center', va='center',
                                fontsize=12, color='#F44336', weight='bold',
        ...
        if self.show_coupling.isChecked():
            for i, k in enumerate(coupling_coeffs):
                ...
                if i == 0:
                    ...
                    self.canvas.axes.text(0, y_pos, f'κ₁ = {k:.3f}', 
                                        ha='center', va='bottom', fontsize=11, color=coupling_color,
                    ...
                    self.canvas.axes.text(0, y_pos, f'κ{i+1} = {k:.3f}', 
                                        ha='center', va='top', fontsize=11, color=coupling_color,
                                        bbox=dict(boxstyle="round,pad=0.3", facecolor='#404040', alpha=0.9))
                else:
                    ...
                    if i < len(ring_positions):
                        ...
                        self.canvas.axes.text(0, y_pos, f'κ{i+1} = {k:.3f}', 
                                            ha='center', va='center', fontsize=11, color=coupling_color,
                                            bbox=dict(boxstyle="round,pad=0.3", facecolor='#404040', alpha=0.9))
...
class InteractiveAnalysisPlot(QWidget):
    ...
    def setup_ui(self):
        ...
        control_panel.setStyleSheet("QFrame { background-color: #404040; border: 1px solid #555; border-radius: 5px; padding: 5px; }")
        ...
        self.plot_selector.setStyleSheet("QComboBox { background-color: #555; color: white; border: 1px solid #777; }")
        ...
        self.show_grid.stateChanged.connect(lambda state: self.log_callback and self.log_callback(f"Grid display {'enabled' if state else 'disabled'}"))
        ...
        self.show_markers.stateChanged.connect(lambda state: self.log_callback and self.log_callback(f"Markers display {'enabled' if state else 'disabled'}"))
        control_layout.addWidget(self.show_markers)
        
        self.export_btn = QPushButton("Export Plot")
        self.export_btn.clicked.connect(self.export_plot)
        self.export_btn.setStyleSheet("QPushButton { background-color: #2196F3; color: white; border: none; padding: 5px 10px; border-radius: 3px; }")
    ...
    def show_placeholder(self):
        """Show enhanced placeholder text"""
        self.canvas.clear_plot()
        self.canvas.axes.text(0.5, 0.5, 'Run Simulation to View\nAnalysis Results',
                             transform=self.canvas.axes.transAxes,
                             ha='center', va='center', fontsize=14, color='#888',
                             bbox=dict(boxstyle="round,pad=0.3", facecolor='#404040', alpha=0.8))
        # Remove title to save vertical space
    ...
    def update_plot(self):
        ...
        self.safe_log(f"Analysis plot changed to: {selected_plot} ({len(wavelengths)} wavelength points)")
        ...
        if selected_plot == "Transmission Spectrum":
            self.canvas.axes.plot(wavelengths, self.current_results['through_dB'], 
                                 '#4CAF50', linewidth=line_width, label='Through Port',
                                 marker=marker_style, markersize=marker_size, alpha=alpha)
            self.canvas.axes.plot(wavelengths, self.current_results['drop_dB'], 
                                 '#F44336', linewidth=line_width, label='Drop Port',
                                 marker=marker_style, markersize=marker_size, alpha=alpha)
            self.canvas.axes.set_ylabel('Transmission (dB)', fontsize=12, color='white')
            # Remove title to save vertical space
            
        elif selected_plot == "Phase Response":
            self.canvas.axes.plot(wavelengths, self.current_results['through_phase'] / np.pi, 
                                 '#2196F3', linewidth=line_width, label='Through Port',
                                 marker=marker_style, markersize=marker_size, alpha=alpha)
            self.canvas.axes.plot(wavelengths, self.current_results['drop_phase'] / np.pi, 
                                 '#FF9800', linewidth=line_width, label='Drop Port',
                                 marker=marker_style, markersize=marker_size, alpha=alpha)
            ...
            self.canvas.axes.plot(wl_delay, self.current_results['through_delay'] * 1e12, 
                                 '#9C27B0', linewidth=line_width, label='Through Port',
                                 marker=marker_style, markersize=marker_size, alpha=alpha)
            self.canvas.axes.plot(wl_delay, self.current_results['drop_delay'] * 1e12, 
                                 '#00BCD4', linewidth=line_width, label='Drop Port',
                                 marker=marker_style, markersize=marker_size, alpha=alpha)
            ...
            self.canvas.axes.plot(wl_disp, self.current_results['through_dispersion'] * 1e12, 
                                 '#795548', linewidth=line_width, label='Through Port',
                                 marker=marker_style, markersize=marker_size, alpha=alpha)
            self.canvas.axes.plot(wl_disp, self.current_results['drop_dispersion'] * 1e12, 
                                 '#607D8B', linewidth=line_width, label='Drop Port',
                                 marker=marker_style, markersize=marker_size, alpha=alpha)
            self.canvas.axes.set_ylabel('Dispersion (ps²/nm)', fontsize=12, color='white')
            # Remove title to save vertical space
            
        elif selected_plot == "Effective Index":
            self.canvas.axes.plot(wavelengths, self.current_results['neff'], 
                                 '#FFC107', linewidth=line_width, label='Effective Index',
                                 marker=marker_style, markersize=marker_size, alpha=alpha)
        ...
        # Enhanced legend
        legend = self.canvas.axes.legend(loc='upper right', frameon=True, fancybox=True, shadow=True,
                                        facecolor='#404040', edgecolor='white')
    ...
    def export_plot(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, "Export Plot", f"ring_analysis_{self.plot_selector.currentText().lower().replace(' ', '_')}.png",
        ...
        if filename:
            ...
            QMessageBox.information(self, "Export Successful", f"High-quality plot saved to {filename}")
...
class ControlPanel(QWidget):
    ...
    def setup_ui(self):
        ...
        self.num_rings_spin.setStyleSheet("QSpinBox { background-color: #555; color: white; border: 1px solid #777; }")
        ...
        spinbox_style = "QDoubleSpinBox { background-color: #555; color: white; border: 1px solid #777; }"
        ...
        self.wl_start_spin.valueChanged.connect(lambda v: self.log_parameter_change("Wavelength Start", v, " nm"))
        ...
        self.wl_stop_spin.valueChanged.connect(lambda v: self.log_parameter_change("Wavelength Stop", v, " nm"))
        ...
        self.wl_res_spin.valueChanged.connect(lambda v: self.log_parameter_change("Wavelength Resolution", v, " nm"))
        ...
        self.loss_spin.valueChanged.connect(lambda v: self.log_parameter_change("Loss", v, " dB/cm"))
        ...
        self.neff_0_spin.valueChanged.connect(lambda v: self.log_parameter_change("Effective Index n₀", v))
        ...
        self.neff_1_spin.valueChanged.connect(lambda v: self.log_parameter_change("Effective Index n₁", v))
        ...
        self.neff_2_spin.valueChanged.connect(lambda v: self.log_parameter_change("Effective Index n₂", v))
    ...
    def update_ring_controls(self):
        ...
        for i in range(num_rings):
            ...
            spin.valueChanged.connect(lambda v, idx=i: self.log_parameter_change(f"Ring {idx+1} Radius", v, " μm"))
        ...
        for i in range(num_rings + 1):
            ...
            spin.valueChanged.connect(lambda v, name=coupling_name: self.log_parameter_change(name, v))
            spin.setStyleSheet(spinbox_style)
        ...
        for i in range(num_rings):
            ...
            spin.setDecimals(3)
            spin.valueChanged.connect(self.parameters_changed)
            spin.valueChanged.connect(lambda v, idx=i: self.log_parameter_change(f"Ring {idx+1} Phase", v, " π rad"))
            spin.setStyleSheet(spinbox_style)
    ...
    def get_configuration(self) -> Dict:
        ...
        return {
            ...
            'neff_coeffs': [self.neff_0_spin.value(), self.neff_1_spin.value(), self.neff_2_spin.value()]
        }
...
class SimulationWorker(QObject):
    ...
    def run_simulation(self, config: Dict):
        ...
        try:
            ...
            neff = (self.system.neff_coeffs[0] + 
...
class RingResonatorStudio(QMainWindow):
    ...
    def update_geometry(self):
        ...
        self.geometry_plot.update_geometry(
            config['ring_radii_um'],
            config['coupling_coeffs'],
            config['phase_shifts_rad']
        )
    ...
    def run_simulation(self):
        ...
        self.add_log_message(f"Simulation parameters: {len(config['ring_radii_um'])} rings, "
