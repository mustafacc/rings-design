...
class TestControlPanel:
    ...
    def test_save_load_config(self, control_panel, tmp_path):
        ...
        # Test loading
        with patch('PySide6.QtWidgets.QFileDialog.getOpenFileName') as mock_load, \
             patch('PySide6.QtWidgets.QMessageBox.information') as mock_info:
...
class TestSimulationWorker:
    ...
    def test_run_simulation(self, worker):
        ...
        if len(finished_signals) == 1:
            ...
            expected_keys = [
                'wavelengths', 'drop_dB', 'through_dB', 'drop_phase',
                'through_phase', 'through_delay', 'drop_delay',
                'through_dispersion', 'drop_dispersion', 'neff'
            ]
